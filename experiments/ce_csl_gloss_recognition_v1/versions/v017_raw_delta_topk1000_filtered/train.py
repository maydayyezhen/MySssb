...
from closed_vocab_dataset import ClosedVocabCeCslGlossDataset  # noqa: E402
...
CLOSED_VOCAB_NAME = "top_k_1000_filtered"
...
CHECKPOINT_DIR = PROCESSED_DIR / "checkpoints" / "full_bilstm_ctc_raw_delta_topk1000_filtered"
LOG_DIR = PROCESSED_DIR / "logs" / "full_bilstm_ctc_raw_delta_topk1000_filtered"
...
def infer_vocab_size_from_closed_vocab() -> int:
    """
    从 v016 top_k_1000 controlled_vocab.json 推断 v017 closed vocab size。
    ...
    if not CONTROLLED_VOCAB_PATH.exists():
        raise FileNotFoundError(f"找不到 controlled vocab 文件：{CONTROLLED_VOCAB_PATH}")

    with CONTROLLED_VOCAB_PATH.open("r", encoding="utf-8") as file:
        controlled_vocab = json.load(file)

    kept_token_count = int(controlled_vocab["keptTokenCount"])

    return kept_token_count + 1
...
def main() -> None:
    ...
    device = get_device()
    vocab_size = infer_vocab_size_from_closed_vocab()

    print("device:", device)
    print("closed_vocab_name:", CLOSED_VOCAB_NAME)
    print("controlled_vocab_path:", CONTROLLED_VOCAB_PATH)
    ...
    train_dataset = ClosedVocabCeCslGlossDataset(
    ...
    dev_dataset = ClosedVocabCeCslGlossDataset(
        dataset_root=DATASET_ROOT,
        split="dev",
        controlled_vocab_path=CONTROLLED_VOCAB_PATH,
    ...
    print("train original sample size:", train_dataset.original_sample_count)
    print("train filtered sample size:", len(train_dataset))
    print("train removed sample size:", train_dataset.removed_sample_count)

    print("dev original sample size:", dev_dataset.original_sample_count)
    print("dev filtered sample size:", len(dev_dataset))
    print("dev removed sample size:", dev_dataset.removed_sample_count)
    ...
    train_config = {
        ...
        "closed_vocab_name": CLOSED_VOCAB_NAME,
        "controlled_vocab_path": str(CONTROLLED_VOCAB_PATH),
        "closed_vocab_size": vocab_size,
        "train_original_sample_count": train_dataset.original_sample_count,
        "train_filtered_sample_count": len(train_dataset),
        "train_removed_sample_count": train_dataset.removed_sample_count,
        "dev_original_sample_count": dev_dataset.original_sample_count,
        "dev_filtered_sample_count": len(dev_dataset),
        "dev_removed_sample_count": dev_dataset.removed_sample_count,
        "seed": SEED,
