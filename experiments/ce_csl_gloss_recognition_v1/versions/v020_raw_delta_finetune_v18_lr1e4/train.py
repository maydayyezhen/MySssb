...
SOURCE_CHECKPOINT_PATH = (
    PROCESSED_DIR
    / "checkpoints"
    / "full_bilstm_ctc_raw_delta_from_topk1000_pretrain"
    / "best_dev_ter.pt"
)

CHECKPOINT_DIR = PROCESSED_DIR / "checkpoints" / "full_bilstm_ctc_raw_delta_finetune_v18_lr1e4"
LOG_DIR = PROCESSED_DIR / "logs" / "full_bilstm_ctc_raw_delta_finetune_v18_lr1e4"
...
def load_full_checkpoint_weights(
    ...
    """
    从 V18 full vocab best_dev_ter.pt 加载完整模型权重。

    说明：
    - V18 和 V20 都是 full vocab，vocab_size 都是 3840。
    - 所以这里加载完整 model_state_dict，包括 output_layer。
    - 不加载旧 optimizer state，因为 V20 是低学习率精修，重新建 optimizer 更稳。
    """
    if not checkpoint_path.exists():
        raise FileNotFoundError(f"找不到源 checkpoint：{checkpoint_path}")
    ...
    report = {
        "sourceCheckpointPath": str(checkpoint_path),
        "sourceEpoch": checkpoint.get("epoch"),
        "sourceTrainLoss": checkpoint.get("train_loss"),
        "sourceDevLoss": checkpoint.get("dev_loss"),
        "sourceDevTER": checkpoint.get("dev_ter"),
        "loadedStateDict": True,
        "loadMode": "full_model_state_dict",
    }
...
def main() -> None:
    ...
    source_load_report = load_full_checkpoint_weights(
        model=model,
    ...
    print("source epoch:", source_load_report["sourceEpoch"])
    print("source dev_TER:", source_load_report["sourceDevTER"])
    print("source dev_loss:", source_load_report["sourceDevLoss"])
    print("load mode:", source_load_report["loadMode"])
    ...
    train_config = {
        ...
        "source_checkpoint_epoch": source_load_report["sourceEpoch"],
        "source_checkpoint_dev_ter": source_load_report["sourceDevTER"],
        "source_checkpoint_dev_loss": source_load_report["sourceDevLoss"],
        "fine_tune_from": "v018_raw_delta_full_vocab_from_topk1000_pretrain_best_dev_ter",
        "seed": SEED,
    ...
    write_json(LOG_DIR / "source_load_report.json", source_load_report)
