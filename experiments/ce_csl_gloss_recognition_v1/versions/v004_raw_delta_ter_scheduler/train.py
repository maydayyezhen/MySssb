"""
CE-CSL BiLSTM-CTC 全量训练脚本 v4：raw_delta + dev_TER scheduler
...
6. 保存最后一轮模型。
7. 写出训练历史日志和最佳预测示例。
8. 使用 dev_TER 驱动学习率调度。

说明：
- 本脚本用于正式全量训练。
- 前面已经通过 overfit_20 和 subset_500 验证训练链路可用。
- 本实验保留 v2 raw_delta 的模型与正则配置，只新增 dev_TER scheduler。
"""
...
CHECKPOINT_DIR = PROCESSED_DIR / "checkpoints" / "full_bilstm_ctc_raw_delta_ter_scheduler"
LOG_DIR = PROCESSED_DIR / "logs" / "full_bilstm_ctc_raw_delta_ter_scheduler"
