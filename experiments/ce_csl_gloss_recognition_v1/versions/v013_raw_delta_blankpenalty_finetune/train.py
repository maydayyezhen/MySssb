...
SOURCE_CHECKPOINT_DIR = PROCESSED_DIR / "checkpoints" / "full_bilstm_ctc_raw_delta_finetune_v2_lr2e4"
SOURCE_CHECKPOINT_PATH = SOURCE_CHECKPOINT_DIR / "best_dev_ter.pt"

CHECKPOINT_DIR = PROCESSED_DIR / "checkpoints" / "full_bilstm_ctc_raw_delta_blankpenalty_finetune"
LOG_DIR = PROCESSED_DIR / "logs" / "full_bilstm_ctc_raw_delta_blankpenalty_finetune"
...
def apply_blank_logit_penalty(
    ...
    """
    Lower blank probability before CTC loss/decode, then renormalize log probs.
    """
    if blank_penalty == 0.0:
        return log_probs_btc

    adjusted_log_probs = log_probs_btc.clone()
    adjusted_log_probs[:, :, blank_id] -= blank_penalty

    return adjusted_log_probs - torch.logsumexp(
        adjusted_log_probs,
        dim=-1,
        keepdim=True,
    )
...
def train_one_epoch(
    ...
    for batch_index, batch in enumerate(dataloader, start=1):
        ...
        log_probs_btc = apply_blank_logit_penalty(
            log_probs_btc=log_probs_btc,
            blank_penalty=BLANK_LOGIT_PENALTY,
            blank_id=BLANK_ID,
        )
...
def main() -> None:
    ...
    print("BLANK_LOGIT_PENALTY:", BLANK_LOGIT_PENALTY)
    ...
    initial_dev_result = evaluate(
        model=model,
        dataloader=dev_loader,
        ctc_loss_fn=ctc_loss_fn,
        device=device,
    )
    initial_dev_loss = float(initial_dev_result["loss"])
    initial_dev_ter = float(initial_dev_result["ter"])
    initial_dev_total_edit_distance = int(initial_dev_result["totalEditDistance"])
    initial_dev_total_target_tokens = int(initial_dev_result["totalTargetTokens"])

    source_metrics["calibrated_dev_loss"] = initial_dev_loss
    source_metrics["calibrated_dev_ter"] = initial_dev_ter
    source_metrics["calibrated_dev_edit_distance"] = initial_dev_total_edit_distance
    source_metrics["calibrated_dev_target_tokens"] = initial_dev_total_target_tokens

    write_json(LOG_DIR / "config.json", full_config)
    write_json(LOG_DIR / "source_checkpoint.json", source_metrics)

    best_dev_ter = initial_dev_ter
    ...
    save_checkpoint(
        ...
        dev_loss=initial_dev_loss,
        dev_ter=initial_dev_ter,
        config=full_config,
    )
    save_checkpoint(
    ...
    write_jsonl(
        ...
        initial_dev_result["predictionExamples"],
    )
    write_jsonl(
    ...
    history_rows: List[Dict] = [
        {
            ...
            "devLoss": initial_dev_loss,
            "devTER": initial_dev_ter,
            "devEditDistance": initial_dev_total_edit_distance,
            "devTargetTokens": initial_dev_total_target_tokens,
            "bestDevTER": best_dev_ter,
            "bestDevLoss": best_dev_loss,
            "epochSeconds": 0.0,
            "elapsedMinutes": 0.0,
            "sourceCheckpoint": True,
            "blankLogitPenalty": BLANK_LOGIT_PENALTY,
        }
    ]
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        if should_eval and epoch - best_dev_ter_epoch >= EARLY_STOP_PATIENCE:
            print(
                ...
                f"(best_epoch={best_dev_ter_epoch}, best_dev_TER={best_dev_ter:.4f})"
            )
            break
