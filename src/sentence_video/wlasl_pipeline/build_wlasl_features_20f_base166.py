# -*- coding: utf-8 -*-
"""
从 WLASL-mini 视频中构建 20 帧 base166 特征。

目的：
对照 plus232 特征，减少额外身体位置/动态差分特征，观察模型是否更依赖手部细节。

base166 每帧特征：
- 左手 78 维：21 点 xyz 相对手腕归一化 + 15 个手部关节角
- 右手 78 维：同上
- 基础身体 10 维：左右肘、左右腕相对肩中心坐标 + 左右肘角度
...
from src.sentence_video.wlasl_pipeline.build_wlasl_features_20f_plus import (
    read_csv_rows,
    write_csv,
    save_json,
    read_all_frames,
...
def extract_frame_feature_base166(result) -> List[float]:
    """
    提取单帧 166 维基础特征。
    ...
    feature = left_hand_feature + right_hand_feature + pose_base_feature

    if len(feature) != 166:
        raise RuntimeError(f"base166 feature dim error: {len(feature)}")

    return feature


def extract_sequence_base166(
    results: List[object],
    frame_indices: List[int],
) -> np.ndarray:
    """
    根据帧下标提取一个样本的 20 帧 base166 特征。
    """
    features = []
    ...
    for frame_index in frame_indices:
        ...
        features.append(extract_frame_feature_base166(result))

    return np.array(features, dtype=np.float32)


def build_one_sample_features_base166(
    holistic,
    video_path: Path,
    target_frames: int,
    padding: int,
...
def main() -> None:
    ...
    parser.add_argument(
        "--output_dir",
        default="D:/datasets/WLASL-mini/features_20f_base166",
        help="特征输出目录",
    )
    parser.add_argument(
        "--target_frames",
    ...
    print("========== 开始构建 WLASL 20f base166 特征 ==========")
    ...
    with mp_holistic.Holistic(
        ...
        for idx, row in enumerate(rows, start=1):
            ...
            try:
                X, meta = build_one_sample_features_base166(
                    holistic=holistic,
                    video_path=video_path,
    ...
    save_json(
        output_dir / "config.json",
        {
            ...
            "labels": labels,
            "feature_note": "166 dims = left_hand78 + right_hand78 + pose_base10; no static_plus, no dynamic_delta",
        },
    )
